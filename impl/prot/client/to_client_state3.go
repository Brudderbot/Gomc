...
type PacketOEntityAnimation struct {
	Entity    ents.Entity
	Animation data.Animation
}

func (p *PacketOEntityAnimation) UUID() int32 {
	return 0x06
}

func (p *PacketOEntityAnimation) Push(writer buff.Buffer, conn base.Connection) {
	writer.PushVrI(int32(p.Entity.EntityUUID()))
	writer.PushByt(byte(p.Animation))
}

type PacketOSpawnPlayer struct {
	EntityID   int64
...
func (p *PacketOSpawnPlayer) UUID() int32 {
	return 0x05
}

func (p *PacketOSpawnPlayer) Push(writer buff.Buffer, conn base.Connection) {
	writer.PushVrI(int32(p.EntityID))
	writer.PushUID(p.PlayerUUID)
	writer.PushPosF(p.Location)
	writer.PushByt(p.Yaw)
	writer.PushByt(p.Pitch)
}
