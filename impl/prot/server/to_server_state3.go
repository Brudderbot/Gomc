...
type PacketIPlayerAnimation struct {
	Hand data.Animation
}

func (p *PacketIPlayerAnimation) UUID() int32 {
	return 0x2A
}

func (p *PacketIPlayerAnimation) Pull(reader buff.Buffer, conn base.Connection) {
	p.Hand = data.Animation(reader.PullVrI())
}
